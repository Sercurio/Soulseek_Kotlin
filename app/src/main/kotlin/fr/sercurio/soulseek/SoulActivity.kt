    ...
    try {
      ...
      soulseekApi =
          object :
            ...
            //                override fun onLogin(isConnected: Boolean, greeting: String?,
            // nothing1: Int?, reason: String?) {
      ...
      lifecycleScope.launch {
        soulseekApi.clientSoul.loginService.login("Airmess", "159753") { loginResponse ->
          if (loginResponse.isConnected) println("HEEEREEE Connected")
          else println("HEREEE Not connected")
          //                viewModel.updateConnected(loginResponse.isConnected)
        }
      }
    } catch (e: Exception) {
      println(e)
...
//  }
  ...
  override fun onQueryChangeListener(query: String?) {
    ...
    if (query != null) {
      ...
      CoroutineScope(Dispatchers.IO).launch { soulseekApi.clientSoul.fileSearch(query) }
    }
  }
  ...
  private fun setCurrentFragment(fragment: Fragment) {
    supportFragmentManager.beginTransaction().apply { replace(R.id.flFragment, fragment).commit() }
  }
  ...
  fun onRoomMessage(roomName: String, username: String, message: String) {
    runOnUiThread { roomFragment.addRoomMessage(RoomMessageApiModel(roomName, username, message)) }
  }
  ...
  fun onUserLeftRoom(roomName: String, username: String) {
    runOnUiThread {
      roomFragment.addRoomMessage(
          RoomMessageApiModel(roomName, "~~~", "$username a quitté la room"))
    }
  }

  fun onRoomList(rooms: ArrayList<RoomApiModel>) {
    runOnUiThread { roomFragment.setRoomList(rooms) }
  }
  ...
  fun onSearchReply(peer: PeerApiModel) {
    runOnUiThread { searchFragment.addSoulFiles(peer) }
  }

package fr.sercurio.saoul_seek.custom_view.expandable_list_view
...
class ExpandableSoulResultsAdapter(private val context: Context, private val listTitles: MutableList<Peer>,
                                   private val listItems: MutableMap<Peer, List<SoulFile>>) : BaseExpandableListAdapter() {
    private val tag = ExpandableSoulResultsAdapter::class.java.toString()
    lateinit var onClickDlButtonListener: ((Peer, SoulFile) -> Unit)
    ...
    override fun getGroupView(listPosition: Int, isExpanded: Boolean,
        ...
        val peerTitle = getGroup(listPosition) as Peer
        if (view == null) {
        ...
        folderPathView.text = peerTitle.soulFiles?.get(0)?.folder ?: "unknown"
        return view
    }
    ...
    override fun getChildView(listPosition: Int, expandedListPosition: Int,
        ...
        dlButton!!.setOnClickListener {
            onClickDlButtonListener.invoke(getGroup(listPosition) as Peer, soulFile)
        }
    ...
    fun addHeaderAndItems(peer: Peer) {
        listTitles.add(peer)

...
class PeerSocket(
    ...
    private val searchReplyService: ResponseCallback<SearchReplyMessage> = ResponseCallback()
) {
  ...
  private val handler = CoroutineExceptionHandler { _, exception ->
    ...
    println("Disconnected of $username")
    socket.close()
    connected = false
  }
  ...
  fun onReceiveSearchReply(callback: (SearchReplyMessage) -> Unit) {
    searchReplyService.subscribe { callback(it) }
  }
  ...
  private suspend fun receiveSearchReply() {
    ...
    if (true /*TODO search the ticket*/) {
      ...
      for (i in 0 until nResults) {
        ...
        soulFiles.add(
            SoulFile(path, filename, folderPath, folder, size, extension, bitrate, vbr, duration))
      }
      ...
      if (soulFiles.isNotEmpty()) {
        searchReplyService.update(
            SearchReplyMessage(soulFiles, slotsFree, avgSpeed, queueLength)
        )
      }
    }
  }
  ...
  private suspend fun receiveTransferReply() {
    ...
    if (allowed && readChannel.packLeft >= 8) {
      ...
      println(
          "Not Allowed:" +
              readChannel.readString()) /*val goSeekService: GoSeekService = this.service
  ...
  */
  ...
  suspend fun queueUpload(filename: String) {
    send(ByteMessage().writeInt32(43).writeStr(filename).getBuff())
  }

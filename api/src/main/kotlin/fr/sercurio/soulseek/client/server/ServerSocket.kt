...
class ServerSocket(
  ...
  private val loginService: ResponseCallback<LoginMessage> = ResponseCallback(),
  private val roomService: ResponseCallback<RoomListMessage> = ResponseCallback(),
  private val peerService: ResponseCallback<ConnectToPeerMessage> = ResponseCallback()
) {
  ...
  fun connect() {
    CoroutineScope(Dispatchers.IO).launch(handler) {
      launch {
            ...
            this@ServerSocket.socket = socket
            this@ServerSocket.writeChannel = socket.openWriteChannel(autoFlush = true)
            this@ServerSocket.readChannel = SoulInputStream(socket.openReadChannel())
          }
          .join()
  ...
  private suspend fun send(message: ByteArray) {
    withContext(Dispatchers.IO) {
      val writeChannel = this@ServerSocket.writeChannel
  ...
  fun onLogin(callback: (LoginMessage) -> Unit) {
    loginService.subscribe(callback)
  }
  ...
  private suspend fun receiveLogin() {
    if (readChannel.readBoolean()) {
      ...
      loginService.update(LoginMessage(true, greeting, ip, null))
    } else {
      val reason: String = readChannel.readString()
      loginService.update(LoginMessage(false, null, null, reason))
    }
  }
  ...
  fun onReceiveConnectToPeer(callback: (ConnectToPeerMessage) -> Unit) {
    peerService.subscribe { callback(it) }
  }
  ...
  private suspend fun receiveConnectToPeer() {
    ...
    peerService.update(ConnectToPeerMessage(username, type, ip, port, token, obfuscatedPort))
  }
  ...
  fun onReceiveRoomList(callback: (RoomListMessage) -> Unit) {
    roomService.subscribe { callback(it) }
  }
  ...
  private suspend fun receiveRoomList() {
    ...
    rooms.addAll(publicRooms) /*
        ...
        */
    roomService.update(RoomListMessage(rooms))
  }
